...
(function() {
    ...
    function updateSlideIndex(increment) { 
        // Update the current slide index by subtracting the increment multiplied by the number of visible slides. 
        slideIndex -= increment * slidesToShow; 
        // Check if the slide index is out of bounds, meaning that the slider has reached the end. 
        if (slideIndex < 0) { 
            // Reset the slide index to the last slide and update the slider position without a transition. 
            slideIndex = slides.length - slidesToShow; 
            slider.style.transition = "none"; 
            slider.style.transform = `translateX(${slideIndex * slideWidth}%)`; 
            // Restore the transition after a brief delay to ensure that the reset is not visible. 
            setTimeout(() => { slider.style.transition = "transform 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94)"; }, 50); } }

        //In addition, you should update the slider.style.transform assignment in the nextSlide and onResize functions to use the slideIndex variable with a negative sign:
